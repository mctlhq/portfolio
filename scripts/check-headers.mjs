#!/usr/bin/env node
// Runtime check (issue #10, P8): HEADs a handful of paths against a live
// instance of the built image and asserts every response carries all eight
// security headers with their expected values, the deliberately missing
// path 404s, and the CSP contains a real sha256- hash (not the build
// placeholder) and no http(s) origin. Invoked from
// .github/workflows/build.yml after `docker run`, since nothing in
...
async function discoverAstroAsset() {
  ...
  if (!match) {
    ...
    return { path: ASTRO_FALLBACK_PATH, expectStatus: 404 };
  }
...
function checkHeaders(url, headers, problems) {
  ...
  if (!csp) {
    ...
    if (!csp.includes('sha256-')) {
      problems.push(`${url}: CSP does not contain sha256-: ${csp}`);
    }
    if (/https?:\/\//.test(csp)) {
...
async function main() {
  ...
  const astroAsset = await discoverAstroAsset();
